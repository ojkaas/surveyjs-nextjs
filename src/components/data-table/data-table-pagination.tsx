  ...
  return (
    <div className='flex items-center justify-between px-2'>
      ...
      <div className='flex items-center space-x-6 lg:space-x-8'>
        ...
        <div className='w-[120px] items-center justify-center text-sm font-medium hidden md:flex'>
          Pagina {table.getState().pagination.pageIndex + 1} van {table.getPageCount()}
        </div>
        <div className='flex items-center space-x-2'>
          <Button variant='outline' className='hidden h-8 w-8 p-0 lg:flex' onClick={() => table.setPageIndex(0)} disabled={!table.getCanPreviousPage()}>
            <span className='sr-only'>Ga naar de eerste pagina</span>
            <DoubleArrowLeftIcon className='h-4 w-4' />
          </Button>
          <Button variant='outline' className='h-8 w-8 p-0' onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()}>
            <span className='sr-only'>Ga naar de vorige pagina</span>
            <ChevronLeftIcon className='h-4 w-4' />
          </Button>
          <Button variant='outline' className='h-8 w-8 p-0' onClick={() => table.nextPage()} disabled={!table.getCanNextPage()}>
            <span className='sr-only'>Ga naar de volgende pagina</span>
          ...
          <Button variant='outline' className='hidden h-8 w-8 p-0 lg:flex' onClick={() => table.setPageIndex(table.getPageCount() - 1)} disabled={!table.getCanNextPage()}>
            <span className='sr-only'>Ga naar de laatste pagina</span>
