...
const ResultPage = async (props: Props) => {
  ...
  const type = session?.user?.type ? (Object.values(UserType).includes(session.user.type as UserType) ? (session.user.type as UserType) : UserType.HUISARTS) : UserType.HUISARTS
